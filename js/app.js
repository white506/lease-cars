(() => {
    ...
    function tabs() {
        ...
        function setTabsStatus(tabsBlock) {
            ...
            const tabsBlockIndex = tabsBlock.dataset.tabsIndex;
            function getTabsAnimationDuration(tabsBlock) {
                if (tabsBlock.hasAttribute("data-tabs-animate")) return tabsBlock.dataset.tabsAnimate > 0 ? Number(tabsBlock.dataset.tabsAnimate) : 500;
                return 0;
            }
            const tabsBlockAnimate = getTabsAnimationDuration(tabsBlock);
            const currentScreenWidth = window.innerWidth;
            if (tabsContent.length > 0) {
                const isHash = tabsBlock.hasAttribute("data-tabs-hash");
                tabsContent.forEach(((tabsContentItem, index) => {
                    if (currentScreenWidth <= 767) if (tabsTitles[index].classList.contains("_tab-active")) {
                        if (tabsBlockAnimate) _slideDown(tabsContentItem, tabsBlockAnimate); else tabsContentItem.hidden = false;
                        if (isHash && !tabsContentItem.closest(".popup")) setHash(`tab-${tabsBlockIndex}-${index}`);
                    } else if (tabsBlockAnimate) _slideUp(tabsContentItem, tabsBlockAnimate); else tabsContentItem.hidden = true; else tabsContentItem.hidden = !tabsTitles[index].classList.contains("_tab-active");
                }));
            }
        }
        function setTabsAction(e) {
            ...
            if (el.closest("[data-tabs-title]")) {
                ...
                const currentScreenWidth = window.innerWidth;
                const shouldCloseTabs = currentScreenWidth <= 767;
                if (isActive && shouldCloseTabs) {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        if (window.location.href.includes("home.html")) header.classList.add("header-invisible");
    }));
    document.addEventListener("DOMContentLoaded", (function() {
        const menuLinks = document.querySelectorAll(".menu__link");
        const currentPage = window.location.pathname.split("/").pop();
        menuLinks.forEach((link => {
            if (link.getAttribute("href") === currentPage) link.classList.add("menu__link-active");
        }));
    }));
    window["FLS"] = true;
    isWebp();
